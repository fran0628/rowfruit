...
router.post("/", async (req, res) => {
  try {
    //拿到前端東西存到變數裡面
    const {user_id, product_id, subscribe_way, receiver, phone, address} = req.body;

    await connection.queryAsync(
      "INSERT INTO subscribe (user_id, product_id, subscribe_way, receiver, phone, address, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIME(),CURRENT_TIME())",
      [user_id, product_id, subscribe_way, receiver, phone, address]
    );
